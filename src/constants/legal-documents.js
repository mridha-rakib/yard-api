const TERMS_AND_CONDITIONS_BODY = `
  <p>
  ...
  <section>
    ...
    <p>
      To use YardWork, you must be at least 18 years old and capable of forming a legally
      binding contract. By creating an account, you agree to:
    </p>
    <ul>
  ...
  <section>
    <h2>4. Job Posting &amp; Acceptance Rules</h2>
    <h3>For Customers</h3>
    <ul>
    ...
    <ul>
      ...
      <li>Honor commitments once you accept a job</li>
      <li>Arrive on time and complete work as described</li>
      <li>Notify customers immediately if you cannot fulfill a commitment</li>
    </ul>
  </section>
  <section>
    ...
    <blockquote>
      <strong>Platform Fee: 12%</strong>
      <p>
        We charge a 12% service fee on all completed transactions. This fee covers payment
        processing, platform maintenance, customer support, and dispute resolution services.
      </p>
    </blockquote>
  ...
  <section>
    ...
    <h3>Full Refunds (100%)</h3>
    <p>
      Customers are eligible for a full 100% refund only under the following conditions:
    </p>
    <ul>
      <li>Property damage directly caused during the service</li>
      <li>YardHero is unable to assign a Hero to the booking request within the active booking period</li>
    </ul>
    <h3>Partial Refunds (90%)</h3>
    <p>
      Customers will receive a 90% refund in situations including, but not limited to:
    </p>
    <ul>
      <li>Incorrect square footage or property size entered during booking</li>
      <li>Additional work or services requested in the job description that were not included in the original booking</li>
      <li>Outside purchases or materials requested for the Hero after booking</li>
      <li>Accidental or mistaken bookings submitted by the customer</li>
      <li>Customer-requested cancellations prior to job completion that are determined to be customer fault</li>
    </ul>
    <p>
    ...
    <ul>
      <li>Once a Hero is actively en route to the property or has begun work, only partial refunds may be issued depending on the circumstances.</li>
      <li>Refund requests submitted after a job has been fully completed may be denied.</li>
      <li>YardHero reserves the right to review all refund requests and determine eligibility on a case-by-case basis.</li>
    </ul>
  </section>
  <section>
    <h2>7. Job Completion &amp; Disputes</h2>
    <p>
    ...
    <ul>
      ...
      <li>Repeated disputes may result in account review or suspension</li>
    </ul>
  </section>
`;

const REFUND_CANCELLATION_POLICY_SECTION = `
  <section>
    <h2>6. Refund &amp; Cancellation Policy</h2>
    <p>
      YardHero is committed to providing a fair experience for both customers and independent Heroes on the platform. Refund eligibility is determined based on the circumstances of the booking.
...
const PRIVACY_POLICY_BODY = `
  <p>
    Your privacy matters to us. We're committed to protecting your personal information and
    being transparent about how we use it.
  </p>
  <section>
    <h2>Information We Collect</h2>
    <p>
      We collect information to provide better services to our users and facilitate
      connections between customers and yard work professionals.
    </p>
    <div>
      <h3>Personal Information</h3>
      <ul>
        <li>Name, email address, and phone number for account creation and communication</li>
        <li>Address and location data to match you with nearby services or opportunities</li>
        <li>Profile photos and work portfolio images you choose to upload</li>
  ...
  <section>
    ...
    <div>
      <h3>Limited Sharing Policy</h3>
      <ul>
        <li>
          <strong>With other users:</strong> Only job-related contact information when you're matched
          for a service
        </li>
        <li>
          <strong>Payment providers:</strong> Necessary information to process payments securely
        </li>
        <li>
  ...
  <section>
    ...
    <div>
      ...
      <div>
        ...
        <p>
          We use cookies to improve your experience and remember your preferences. You can
  ...
  <section>
    ...
    <div>
      ...
      <div>
        <h3>Update</h3>
        <p>Modify or correct your personal information through your account settings</p>
      </div>
      <div>
        <h3>Delete</h3>
  ...
  <section>
    ...
    <p>
      ...
      we'll notify you through email or a prominent notice on our platform. We encourage
      you to review this policy periodically to stay informed about how we protect your
      information.
    </p>
  </section>
  <section>
    ...
    <p>
      <a href="mailto:privacy@yardpro.com">privacy@yardpro.com</a>
...
const DEFAULT_LEGAL_DOCS = [
  {
    ...
    status: "active",
    body: TERMS_AND_CONDITIONS_BODY.trim(),
  },
  {
    id: "privacy-policy",
    name: "Privacy Policy",
    status: "active",
    body: PRIVACY_POLICY_BODY.trim(),
  },
  {
    id: "cookie-policy",
    name: "Cookie Policy",
...
const PLACEHOLDER_LEGAL_BODY_PATTERN = /(what is lorem ipsum|lorem ipsum|content here)/i;
...
const LEGACY_AUTOGENERATED_PRIVACY_PATTERNS = [
  ...
  /Send occasional offers or service updates if you choose to receive promotional messages\./i,
  /Need help understanding how your information is used\? Reach out to our support team/i,
];

const getDefaultLegalDocs = () => DEFAULT_LEGAL_DOCS.map((document) => ({ ...document }));

const isPlaceholderLegalBody = (value = "") =>
  PLACEHOLDER_LEGAL_BODY_PATTERN.test(String(value || "").trim());

const isLegacyAutoGeneratedLegalBody = (documentId = "", value = "") => {
  ...
  if (String(documentId || "").trim() === "terms-of-service") {
    return LEGACY_AUTOGENERATED_TERMS_PATTERNS.every((pattern) => pattern.test(normalizedValue));
  }

  if (String(documentId || "").trim() === "privacy-policy") {
    return LEGACY_AUTOGENERATED_PRIVACY_PATTERNS.every((pattern) => pattern.test(normalizedValue));
  }

  return false;
};

const hasRefundCancellationPolicySection = (value = "") =>
  /Refund\s*&(?:amp;)?\s*Cancellation Policy/i.test(String(value || ""));

const addRefundCancellationPolicySection = (value = "") => {
  const body = String(value || "").trim();

  if (!body || hasRefundCancellationPolicySection(body)) {
    return body;
  }

  const renumberedBody = body.replace(
    /(<h2\b[^>]*>\s*)6\.\s*(Job Completion\s*&(?:amp;)?\s*Disputes\s*<\/h2>)/i,
    (_match, prefix, titleEnd) => `${prefix}7. ${titleEnd}`
  );
  const paymentsSectionPattern =
    /(<section\b[^>]*>\s*<h2\b[^>]*>\s*5\.\s*Payments\s*&(?:amp;)?\s*Platform Fees\s*<\/h2>[\s\S]*?<\/section>)/i;

  if (paymentsSectionPattern.test(renumberedBody)) {
    return renumberedBody.replace(
      paymentsSectionPattern,
      (paymentsSection) => `${paymentsSection}\n${REFUND_CANCELLATION_POLICY_SECTION.trimEnd()}`
    );
  }

  return `${renumberedBody}\n${REFUND_CANCELLATION_POLICY_SECTION.trimEnd()}`.trim();
};

const normalizeTermsLegalBody = (documentId = "", value = "") => {
  const body = String(value || "").trim();
  ...
  return addRefundCancellationPolicySection(body);
};

module.exports = {
  DEFAULT_LEGAL_DOCS,
  PRIVACY_POLICY_BODY,
  REFUND_CANCELLATION_POLICY_SECTION,
  TERMS_AND_CONDITIONS_BODY,
  addRefundCancellationPolicySection,
  getDefaultLegalDocs,
